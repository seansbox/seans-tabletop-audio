...
def download_file(c, url, dir=os.getcwd(), name=None, user_agent=USER_AGENTS["ttf"]):
    ...
    Parameters:
        ...
        dir (str): Destination directory for the downloaded file. Defaults to the working directory.
        name (str): Destination file name for the downloaded file. Defaults to the name of the file from the URL.
        user_agent (str): User-Agent string to mimic a specific browser (default is USER_AGENTS['ttf']).
    """
...
@task
def download_font(c, url, slug, pubdir="../public", fixfunc=None):
  """
  Downloads and processes a font CSS file and its font files, with optional path fixing for font URLs.

  Parameters:
      folder (str): Name of the folder to create within the TTF directory.
      csspath (str): URL path to the CSS file.
      relpath (str, optional): Optional relative path to prefix to each font URL.
      pathfix (str, optional): Path prefix to replace in each URL before downloading.
  """
  css_path = f"{pubdir}/ttf/{slug}"
  download_file(c, url, css_path, name=f"{slug}.css")
  ...
  with open(css_file_path, "r") as css_file:
    ...
    for font_url in font_urls:
        ...
        download_file(c, font_url, css_path)
